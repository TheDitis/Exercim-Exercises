#[cfg(test)]
mod tests {
    use sublist::{sublist, Comparison, sublist_inds};

    #[test]
    fn sublist_inds_test() {
        assert_eq!(Some((0, 2)), sublist_inds(&[1, 2, 3], &[1, 2, 3, 4, 5]))
    }

    #[test]
    fn sublist_inds_test2() {
        assert_eq!(Some((2, 4)), sublist_inds(&[1, 2, 3], &[-1, 0, 1, 2, 3, 4, 5]))
    }

    #[test]
    fn sublist_inds_test3() {
        assert_eq!(None, sublist_inds(&[0, 2, 3], &[1, 2, 3, 4, 5]))
    }

    #[test]
    fn sublist_inds_test4() {
        assert_eq!(Some((0, 0)), sublist_inds(&[], &[1]))
    }

    #[test]
    fn sublist_inds_test5() {
        assert_eq!(Some((0, 0)), sublist_inds(&[1], &[1]))
    }
    #[test]
    fn sublist_inds_test6() {
        assert_eq!(Some((1, 3)), sublist_inds(&[1, 1, 2], &[1, 1, 1, 2]))
    }
