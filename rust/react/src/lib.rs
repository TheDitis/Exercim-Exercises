...
impl<'a, T: 'a + Copy + PartialEq> Reactor<'a, T> {
    ...
    fn update_deps(&mut self, id: CellId) -> HashSet<ComputeCellId> {
        let mut have_changed = HashSet::new();
        let mut ids = vec![id];
        while !ids.is_empty() {
            let id = ids.remove(0);
            if let Some(dependents) = self.dependency_map.get(&id) {
    ...
    pub fn add_callback<Fm: 'a + Fn(T)>(
        ...
        if let Some(comp_cell) = self.compute_cells.get_mut(&id) {
            let cb_id = CallbackId(generate_id());
            comp_cell.callbacks.insert(cb_id, Box::new(callback));
            Some(cb_id)
        } else { None }
    }
